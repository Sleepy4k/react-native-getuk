...
const EditReviewShop = ({ route, navigation }) => {
  const { store, review } = route.params.param;

  if (!store || !review) {
    notification('store or review data not found', 'Error');
  ...
  const [state, setState] = useState({
    ...
    shopDescription: review?.description
  });
  ...
  const handleSave = async () => {
    try {
      ...
      await reviewModel.updateReview(review.id, {
        store: store.id,
  ...
  return (
    <MainLayout containerStyle={styles.container}>
      <View style={styles.nav}>
        ...
        <Text style={styles.screenTitle}>Edit Review</Text>
...
EditReviewShop.propTypes = {
  navigation: PropTypes.object.isRequired,
};

EditReviewShop.defaultProps = {
  navigation: {
    replace: () => {}
  },
};

export default EditReviewShop;
